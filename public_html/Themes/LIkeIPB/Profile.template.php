...
function template_summary()
{
	...
	echo '
	<div id="profileview" class="st_profile">
		<div class="cover', (!empty($context['member']['id_cover'])? '" style="background-image: url(' . $context['member']['st_profile_cover']['href'] . ')"' : '"'), '>
	...
	if (!empty($context['print_custom_fields']['before_member']))
		foreach ($context['print_custom_fields']['before_member'] as $field)
			if (!empty($field['output_html']))
				...
				echo '
								<a href="', $scripturl, '?action=profile;u=', $context['member']['id'], '">
									', $context['member']['name'], '
								</a>
								<span id="userstatus">
									', $context['can_send_pm'] ? '<a href="' . $context['member']['online']['href'] . '" title="' . $context['member']['online']['text'] . '" rel="nofollow">' : '', $settings['use_image_buttons'] ? '<span class="' . ($context['member']['online']['is_online'] == 1 ? 'on' : 'off') . '" title="' . $context['member']['online']['text'] . '"></span>' : $context['member']['online']['label'], $context['can_send_pm'] ? '</a>' : '', '
								</span>';
	...
	if (!empty($context['print_custom_fields']['above_signature']))
	{
		...
		if (!empty($fields))
			echo '
			<div class="custom_fields_above_signature windowbg">
	...
	if (!empty($context['print_custom_fields']['below_signature']))
	{
		...
		if (!empty($fields))
			echo '
			<div class="custom_fields_below_signature windowbg">
				<ul class="nolist">', $fields, '
				</ul>
			</div>';
	}
