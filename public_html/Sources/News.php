...
function getXmlNews($xml_format, $ascending = false)
{
	...
	while (!$done)
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT
				m.smileys_enabled, m.poster_time, m.id_msg, m.subject, m.body, m.modified_time, t.id_first_msg,
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		if (!empty($modSettings['seo_enable_linkback']) && $row['id_first_msg'] == $row['id_msg'])
			$row['body'] .= '<br /><br /><span class="smalltext">' . $txt['seo_linkback'] . ': ' . $scripturl . '?topic=' . $row['id_topic'] . '.0</span>';
...
function getXmlRecent($xml_format)
{
	...
	$request = $smcFunc['db_query']('', '
		SELECT
			m.smileys_enabled, m.poster_time, m.id_msg, m.subject, m.body, m.id_topic, t.id_board, t.id_first_msg,
