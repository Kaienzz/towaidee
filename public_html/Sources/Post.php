...
function Post($post_errors = array())
{
	...
	if (!empty($topic))
	{
		$request = $smcFunc['db_query']('', '
			SELECT
				t.locked, t.approved, COALESCE(ln.id_topic, 0) AS notify, t.is_sticky, t.id_poll, t.id_last_msg, mf.id_member, t.meta_keyword, t.meta_desc, t.meta_noindex,
		...
		list ($locked, $topic_approved, $context['notify'], $sticky, $pollID, $context['topic_last_message'], $id_member_poster, $context['meta_keywords'], $context['meta_desc'], $context['meta_noindex'], $id_first_msg, $first_subject, $editReason, $lastPostTime) = $smcFunc['db_fetch_row']($request);
		$smcFunc['db_free_result']($request);
...
function Post2()
{
	...
	// SMFPacks SEO Pro
	if (!empty($modSettings['seo_custom_meta']) && allowedTo('post_custom_meta') && ($newTopic || (!empty($_REQUEST['msg']) && isset($topic_info) && $topic_info['id_first_msg'] == $_REQUEST['msg'])))
	{
		$msgOptions['meta_desc'] = !empty($_POST['meta_desc']) ? $_POST['meta_desc'] : '';
		$msgOptions['meta_keywords'] = !empty($_POST['meta_keywords']) ? $_POST['meta_keywords'] : '';
		$msgOptions['meta_noindex'] = !empty($_POST['meta_noindex']) ? 1 : 0;
	}
