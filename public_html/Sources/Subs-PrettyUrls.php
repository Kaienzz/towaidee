...
function pretty_generate_url($text)
{
	...
	if ($encoding != 'UTF-8' && function_exists('iconv'))
		$text = iconv($encoding, 'UTF-8', $text);
	else if ($encoding != 'UTF-8' && function_exists('mb_convert_encoding')) //	If the database encoding isn't UTF-8 and multibyte string functions are available, try converting the text to UTF-8
		$text = mb_convert_encoding($text, 'UTF-8', $encoding);
	else if ($encoding == 'ISO-8859-9') //	Fix Turkish
	{
